...
# def parallel_node2vec(c_partition, params, G):#workers = 2,jump = 4)
#     G_sub= G.subgraph(c_partition)
...
def partition_community_dict(community_dict,G,data_name,CD_model,seed,thres=10000,to_save=True):
    ...
    if to_save:
        ...
        # save_to_gzip(c_to_j,'community_to_partition_{}_{}_{}.gz'.format(data_name,CD_model,seed))
...
def set_community_to_G(community_dict,G):
    G_subs = {}
    for c,nodes in community_dict.items():
        G_sub = G.subgraph(nodes)
        G_sub = nx.relabel_nodes(G_sub, lambda x: '{}_{}'.format(x,c))
        G_subs[c] = G_sub

    return G_subs

def node2embeddings_community(community_dict,c_to_partition,best_params_i,G):#,to_max = False): #is_community_level=False
    #create a community-level node2vec, and then merge all node2vecs to a single dictionary
    n2v_dict = {}
    workers = 4  # cpu_count()
    G_subs = set_community_to_G(community_dict,G)
    ###### TO COMPLETE #####
    ########################
    return n2v_dict
...
def create_train_test_community(model,data_obj,bin_op,dim,only_validate = False,communities=None):
    ...
    if not only_validate:
        ...
        # if only_validate:
        data_within_pos = model.edge2vec_community(
        list(((c, edge[0]), (c, edge[1])) for dict_i in communities for c in dict_i.keys() for edge in data_obj['pos']['within'][c] ), bin_op, dim, True)
        data_within_neg = model.edge2vec_community(
        list(((c, edge[0]), (c, edge[1])) for dict_i in communities for c in dict_i.keys() for edge in data_obj['neg']['within'][c] ),bin_op,dim,False)
        # else:
        #     data_within_pos = model.edge2vec(list(chain(*data_obj['pos']['within'].values())),bin_op,dim,True)
        #     data_within_neg = model.edge2vec(list(chain(*data_obj['neg']['within'].values())),bin_op,dim,False)
        data_within = merge_edges(data_within_pos,data_within_neg)
...
def exe_community(files_name,data_name,CD_model,model_name,has_two_learners,has_communities,is_community_level,seed,
    ...
    counter = 1
    dflt_params = [1.0,1.0,128,20,30,12]
    ...
    for i in range(num_partitions):
        ...
        for combination in all_combinations:
            ...
            n2v_params =  [i]+[params for params in best_params.values()]
            n2v_dict = node2embeddings_community(community_dict,c_to_partition,best_params,G)
            train_dict,test_dict = nodes2edge_community(n2v_dict,files_name,CD_model,data_name,seed,has_communities,
                                                        True,best_params)

            link_prediction(model_name,False,is_community_level,train_dict,test_dict,model_params_path,
                            n2v_params,output_val,only_validate=True) #has_two_learners = False
        ...
        best_params.extend(best_params_i)
