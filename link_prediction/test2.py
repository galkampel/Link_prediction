...
class Splitter(object):
    def __init__(self,method,num_epochs = 100,step = 10,seed=0, save_best_model=True):
        self.method = method
    ...
    def calc_epoch_loss(self,model,loss,loader):
        ...
        for X, y in loader:
            ...
            size += batch_size
            fx = model(X_var)
            print('fx shape = {}\ny_var shape = {}'.format(fx.size(),y_var.size()))
    ...
    def set_within_between_sep(self,w_links,b_links,params_path='split_wb.json',frac_between = 0.75,frac_within=0.8):
        ...
        print('links type = {}\nfrac = {}'.format(type(b_links),frac_between))
        ...
        loader_val = get_loader(np.vstack((w_test[:,:-1],b_test[:,:-1])),np.vstack((w_test[:,-1].reshape((-1,1)),b_test[:,-1].reshape((-1,1)))))
        ...
        for optimizer_name in self.optimizers_name:
            for params in params_dict[optimizer_name]:
                ...
                if not has_diverged:
                    ...
                    self.train_val_plot(train_loss_lst,'plots_edge2vec/{}_{}.png'.format(self.method,optim_str))
