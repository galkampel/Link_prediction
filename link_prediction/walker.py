...
split_list = lambda lst,num_chunks : list(lst[i::num_chunks] for i in range(num_chunks))
...
class DeepWalker:
    ...
    def parallel_walk(self,iters,walk_length):
        nodes = list(self.G.nodes())
        N = len(nodes)
        walks = [None]* (N *iters)
        for i in range(iters):
            random.shuffle(nodes)
            walks[i*N:(i+1)*N] = [self.node2vec_walk(walk_length,start_node=node) for node in nodes]  # expand_node(get_node(node),get_random_label(self.G,node))
        return walks


    def simulate_walks(self, num_walks, walk_length,calc_nodes2idx= True,num_cores = 4 ): #is_community_level
        ...
        start = time()
        par_walks = Parallel(n_jobs=num_cores)(delayed(self.parallel_walk)(par_iters,walk_length)
                                           for par_iters in iters)
        walks = list(chain.from_iterable(par_walks))
        print('it took {:.3f} seconds to create walks'.format(time()-start))
        if calc_nodes2idx:
            nodes2idx = self.get_nodes_locations(walks)
            return walks,nodes2idx
        else:
            return walks
    ...
    def preprocess_transition_probs(self,n_jobs=cpu_count()):
        G = self.G
        num_chunks_nodes = n_jobs
        ...
        print('finished creating alias nodes dict')
        alias_nodes = {}
    ...
    def get_primal_locations(self,walks):
        nodes2idx = {}
        for node in self.G.nodes():
            for j,walk in enumerate(walks):
                if walk[0] == node:
                    nodes2idx.setdefault(node,[]).append((j,0))
        return nodes2idx

    def get_nodes_locations(self,walks):
        nodes2idx = {}
        start = time()
        for j,walk in enumerate(walks):
            for k, word in enumerate(walk):
                nodes2idx.setdefault(word,[]).append((j,k))
        print('it took {:.3f} seconds to create node2idx dict'.format(time()-start))

        print('nodes2idx has {} nodes and {} words'.format(len(nodes2idx.keys()),len(list(chain.from_iterable(nodes2idx.values())))))
        return nodes2idx
...
class Walker:
    ...
    # def parallel_walk(self,walk_length,nodes,):#,walks):
    #     start = time()
    ...
    def parallel_walk(self,walk_length,nodes):
        start = time()
        walks = [self.node2vec_walk(walk_length=walk_length, start_node=node) for node in nodes]
        print('it took {:.3f} seconds to create a walk'.format(time()-start))
        return walks
    ...
    def simulate_walks(self, num_walks, walk_length,num_cores = 3 ): #is_community_level
        ...
        for i in range(num_walks):
            random.shuffle(nodes)
            walks_iter = [self.node2vec_walk(walk_length,node) for node in nodes]
            walks[i*N:(i+1)*N] = walks_iter
...
class CommunityWalker:
    ...
    def parallel_walk(self,par_iters, to_expand):
        start = time()
        N = self.N
        nodes = self.nodes
        walks = [None]* (N *par_iters)
        for i in range(par_iters):
            random.shuffle(nodes)
    ...
    def simulate_walks(self, num_walks, to_expand, num_cores=4): #num_cores=3
        '''
        Repeatedly simulate random walks from each node.
        '''
        # iters = [num_walks // num_cores] * num_cores
        ...
        # print('iters:\n{}'.format(iters))
        # num_cores = cpu_count()
        self.nodes, self.N = self.get_all_nodes()
        # args = [(par_iters, to_expand) for par_iters in  iters]
        # # for i,el in enumerate(chunk_nodes_lst):
        # #     print('chunk {} is of size of {}'.format(i,len(el)))
        # pool = Pool(processes = 3 )
        # par_walks = pool.starmap(self.parallel_walk, args)
        # print('walk length = {}'.format(len(par_walks)))
        # pool.close()
        # pool.join()
        par_walks = Parallel(n_jobs=4)(delayed(self.parallel_walk)(par_iters,to_expand)
                                       for par_iters in iters)
        walks = list(chain.from_iterable(par_walks))
    ...
    def preprocess_transition_probs(self,multiplier,n_jobs=cpu_count()):
        ...
        G = self.G
        self.multiplier = multiplier
        num_chunks_edges = int(n_jobs * multiplier )  # multiplier *  1.2
        num_chunks_nodes = int(n_jobs / 6)
        sliced_nodes = sliced_el(list(G.nodes()),num_chunks_nodes)
        alias_nodes_lst = Parallel(n_jobs=num_chunks_nodes)(
            delayed(self.parallel_nodes)(nodes) for nodes in sliced_nodes)
