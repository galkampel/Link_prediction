...
def partition_community_dict(data_name,CD_model,has_communities,seed,files_name,thres=10000,to_save=True):
    ...
    for i,k_med_dia in enumerate(k_med_dias):
        ...
        if num_edges > thres:
            num_edges = 0
            partition.append(dict_i)
            dict_i = {}
        elif i == N-1:
            partition.append(dict_i)

    for c in c_outlies:
        partition.append({c: community_dict[c]})
        ordered_communities.append({c:community_dict[c]})
...
def node2embeddings_community(c_partition_i,params,G,data_name,files_name,CD_model,seed):#,to_max = False): #is_community_level=False
    ...
    for i,(c,nodes) in enumerate(c_partition_i.items()):
        # if len(nodes) > 450:
        #     workers = cpu_count()
        # print('params_i:\n{}'.format(params[i]))
        n2v_dict.update(parallel_node2vec(c,nodes,params[keys[i]],G))

    # if to_max:
    #     community_dict = read_gzip_object('{}_{}_{}.gz'.format(data_name,CD_model,files_name[1]))
    #     n2v_dict = max_n2v(n2v_dict,G,community_dict)
    # else:
    #     n2v_dict = adjust_n2vdict(dict(n2v_dict))
    # print('There are {} nodes'.format(len(G.nodes())))
    # print('There are {} keys'.format(len(n2v_dict.keys())))
    # save_to_gzip(dict(n2v_dict),'{}_{}_{}_{}_{}_{}_{}.gz'.format(data_name,params_str,CD_model,int(has_communities),
    #                                                                    int(is_community_level),seed,files_name[-2]))
    return n2v_dict
...
def create_train_test_community(model,data_obj,bin_op,dim,only_validate = False,communities=None):
    ...
    if not only_validate:
        data_within_pos = model.edge2vec_community([(c,data_obj['pos']['within'][c]) for c in communities.keys()],bin_op,dim,True)
        data_within_neg = model.edge2vec_community([(c,data_obj['neg']['within'][c]) for c in communities.keys()],bin_op,dim,False)
        data_within = merge_edges(data_within_pos,data_within_neg)
        data_between_pos = model.edge2vec_community([(c,data_obj['pos']['between'][c]) for c in communities.keys()],bin_op,dim,True)
        data_between_neg = model.edge2vec_community([(c,data_obj['neg']['between'][c]) for c in communities.keys()],bin_op,dim,False)
        data_between = merge_edges(data_between_pos,data_between_neg)
        return data_within,data_between
    else:
        # data_within_pos,data_within_neg = None,None
...
def exe(files_name,num_to_save,data_name,CD_model,model_name,has_two_learners,has_communities,is_community_level,
        seed,model_params_path=None,num_combinations = 120):
...
def exe_community(files_name,data_name,CD_model,model_name,has_two_learners,has_communities,is_community_level,seed,
                  model_params_path=None,num_to_save=12):
    ...
    all_combinations = [(ps[i],qs[j],d,r,l,k) for i  in range(len(ps)) for j in range(i,len(qs)) for l in ls for k in ks]
    keys = ("p","q","d","r","l","k")
    N = len(keys)
    ...
    for i in range(num_partition):
        best_all_params = {j:{"p":1,"q":1,"d":64,"r":16,"l":50,"k":5} for j in range(len(c_partition[i]))}
        for j in range(len(c_partition[i])):
            print('i = {}, j = {}'.format(i,j))
            best_params_j = {}
            best_val = 0
            flag = True
            n2v_most_dict = {} #all except from the
            for combination in all_combinations:
                n2v_dict = {}
                best_all_params[j] = {keys[n]:combination[n] for n in range(N)}
                n2v_params =  [(i,j)]+list(best_all_params[j].values())
                if flag:
                    n2v_most_dict = node2embeddings_community({c:nodes  for idx,(c,nodes) in  enumerate(c_partition[i].items()) if idx != j},{idx:params for idx,params in best_all_params.items() if idx != j},G,data_name,files_name,CD_model,seed)
                    n2v_last_dict = node2embeddings_community({c:nodes  for idx,(c,nodes) in enumerate(c_partition[i].items()) if idx == j},{idx:params for idx,params in best_all_params.items() if idx == j},G,data_name,files_name,CD_model,seed)
                    flag = False
                else:
                    n2v_last_dict = node2embeddings_community({c:nodes  for idx,(c,nodes) in enumerate(c_partition[i].items()) if idx == j},{idx:params for idx,params in best_all_params.items() if idx == j},G,data_name,files_name,CD_model,seed)

                n2v_dict.update(n2v_most_dict)
                n2v_dict.update(n2v_last_dict)
                ...
                if val_acc > best_val:
                    best_val = val_acc
                    best_params_j = best_all_params[j]
                if counter % num_to_save == 0:
                    save_res_to_csv(int(counter / num_to_save),data_name,CD_model,has_communities,
            ...
            best_all_params[j] = best_params_j
            best_params.append(best_params_j)
...
def exe_community_test(files_name,num_to_save,data_name,CD_model,model_name,has_communities,is_community_level,seed,
    ...
    G = get_G(data_name,CD_model,int(has_communities),seed,files_name)
    print('Node2vec phase (community-wise)')
    n2v_dict = node2embeddings_community(ordered_communities,best_all_params,G,data_name,files_name,CD_model,seed)
    train_dict,test_dict = nodes2edge_community(n2v_dict,files_name,CD_model,data_name,seed,has_communities,
    ...
    if not is_community_level:
        ...
        if only_validate:
            counter,output = exe_community(files_name,data_name,CD_model,model_name,has_two_learners,
                                           has_communities,is_community_level, seed,model_params_path)
        else:
            counter,output = exe_community_test(files_name,num_to_save,data_name,CD_model,model_name,has_communities,
