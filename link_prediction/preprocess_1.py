...
### split negative labels to randomize and in-cluster
    ...
    def parallel_between_edges(self,communities_chunk,community_dict,mst_edges,test_neg_edges,test_pos_edges):
        print('num communities:\t{}'.format(len(communities_chunk)))
        tmp_dict_neg = {}
        tmp_dict_pos = {}
        ...
        for communities,edges in communities_chunk.items():
            ...
            tmp_dict_pos[communities] = self.create_positive_edge_between(edges,mst_edges,num_edges)
...
## filename- the file to save the raw test set
    def get_outlier(self):
        ...
        print('threshold for max links per C = {}'.format(thres))
        print('C outliers:\n{}'.format(df[df['links in Community C'] >= thres]["C"].tolist()))
    ...
    def set_new_G(self,filename):
        ...
        #     for sub_graph in nx.connected_component_subgraphs(G_sub):
        #         sub_graph_num_edges = len(sub_graph.edges())
        #         if sub_graph_num_edges == 0:
        #             bad_components += 1
        #             for u in sub_graph.nodes():
        #                 print('label {} is not good for {}'
        #                       '\n u labels = {}'.format(c,u,G.nodes[u]['label']))
        #         num_edges += len(sub_graph.edges())
        #
        #         # print('subgraph has {} edges'.format())
        #
        # print('There are {} bad compnents'.format(bad_components))
        # exit()
        ...
        self.mst_edges = set(nx.minimum_spanning_edges(G,data=False))
        print('mst has {} edges'.format(len(mst_edges)))
        all_edges = list(G.edges())
        print('all edges before removal = {}'.format(len(all_edges)))
        all_edges = list(set(all_edges).difference(mst_edges))
        print('all edges after removal = {}'.format(len(all_edges)))
        ...
        new_test_edges = {c:[] for c in test_pos_edges['within'].keys()}
        num_problematic_edges = 0
        for edges in test_pos_edges['within'].values():
            for edge in edges:
                if edge not in mst_edges:
                    for c2 in set(G.nodes[edge[0]]['label']).intersection(set(G.nodes[edge[1]]['label'])):
                        new_test_edges[c2].append(edge)
                else:
                    num_problematic_edges += 1
        print('{} edges were removed from positive within'.format(num_problematic_edges))
        test_pos_edges['within'] = new_test_edges
        ...
        Parallel(n_jobs=self.num_cores)(delayed(self.parallel_between_edges)(communities_chunk,community_dict,mst_edges,
    ...
    def create_positive_edge_within(self, subgraph,num_pos_edges ,max_size):
        ...
        cand_edges = list(set(subgraph.edges()).difference(mst_edges))
        size = len(cand_edges)
        ...
        if nx.number_connected_components(G_copy) == 1:
            ...
            return rel_edges,num_pos_edges
        else:
            print('error!!!!')
            exit()


    def create_positive_edge_between(self,edges,mst_edges,num_edges):
        rel_edges = list(set(edges).difference(mst_edges))
