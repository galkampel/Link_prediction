...
class Node2vec(object):
    ...
    def __init__(self, G, params,community_dict,multiplier,is_community_level=False,
        ...
        if is_community_level: # and not to_intertwine:
            start = time()
            self.walker.preprocess_transition_probs(multiplier) #kwargs["workers"]
            print('it took {:.3f} seconds to create transition probs'.format(time()-start))
            start = time()
