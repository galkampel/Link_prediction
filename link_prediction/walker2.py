...
class Walker:
    ...
    def parallel_edges(self,edges):
        ...
        return alias_edges
    def preprocess_transition_probs(self,num_chunks=cpu_count()):
        '''
        ...
        sliced_nodes = sliced_el(list(G.nodes()),num_chunks)
        alias_nodes_lst = Parallel(n_jobs=n_jobs)(
            delayed(self.parallel_nodes)(nodes) for nodes in sliced_nodes)
        alias_nodes = {}
        sliced_edges = sliced_el(list(G.edges()),num_chunks)
...
class CommunityWalker:
    ...
    def node2vec_walk(self,start_node,to_expand,rho):
        '''
        Simulate a random walk starting from start node.
        '''
        alias_nodes = self.alias_nodes
        ...
        while len(walk) < self.params["l"]:
            if len(walk) == 1:
                ...
                v_cur = cur_nbrs[alias_draw(alias_nodes[(v_cur)][0], alias_nodes[(v_cur)][1])]
                v_Cs = list(G.nodes[v_cur]['label'])
                vals = [1 - rho if start_c == c else rho for c in v_Cs]
                norm_const = sum(vals)
    ...
    def get_all_nodes(self):
        ...
        for c,nodes in community_dict.items():
            for node in nodes:
                all_nodes[t] = expand_node(node,c)
                t += 1
        return all_nodes,N

    def parallel_walk(self, iters, to_expand, rho):  # ,walks):
        # start = time()
        nodes, N = self.get_all_nodes()
        walks = [None] * (N * iters)
        for i in range(iters):
            random.shuffle(nodes)
            walks[i * N: (i + 1) * N] = [self.node2vec_walk(start_node=node, to_expand=to_expand, rho=rho) for node in
                                         nodes]  # expand_node(get_node(node),get_random_label(self.G,node))
        # print('it took {:.3f} seconds to create walk for all nodes'.format(time()-start))
        return walks
    ...
    def simulate_walks(self, num_walks, to_expand, num_cores=3):
        ...
        # print('iters:\n{}'.format(iters))
        # num_cores = cpu_count()
        walks = Parallel(n_jobs=num_cores)(delayed(self.parallel_walk)(par_iters, to_expand, rho)
                                           for par_iters in iters)
        walks = list(chain(*walks))
        # print('it took {:.3f} seconds to create walks'.format(time()-start))
        return walks
    ...
    def get_alias_edge(self,alias_edges,src, dst):
        ...
        norm_const = sum(unnormalized_probs)
        normalized_probs =  [float(u_prob)/norm_const for u_prob in unnormalized_probs]
        alias_edges[(src, dst)] = alias_setup(normalized_probs)
        return
    ...
    def preprocess_transition_probs(self,num_chunks=cpu_count()):
        ...
        sliced_nodes = sliced_el(list(G.nodes()),num_chunks)
        alias_nodes_lst = Parallel(n_jobs=num_chunks)(
            delayed(self.parallel_nodes)(nodes) for nodes in sliced_nodes)
        sliced_edges = sliced_el(list(G.edges()),num_chunks)
        alias_edges_lst = Parallel(n_jobs=num_chunks)(
            delayed(self.parallel_edges)(edges) for edges in sliced_edges)
        alias_nodes = {}
        alias_edges = {}
        for i in range(num_chunks):
